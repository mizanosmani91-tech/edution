      ...
      <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="max-width:520px;background:#FFFFFF;border-radius:18px;overflow:hidden;box-shadow:0 20px 40px -20px rgba(31,36,50,.3);">
        ...
        <tr>
          <td style="padding:32px;">
            <h1 style="margin:0 0 10px;font-size:20px;color:#1F2432;">আবেদন গ্রহণ করা হয়েছে</h1>
            <p style="margin:0 0 20px;font-size:14.5px;line-height:1.8;color:#6B7280;">
              প্রিয় {{ $institution->admin_name }},<br><br>
              <strong style="color:#1F2432;">{{ $institution->name }}</strong>-এর জন্য আপনার EDUTION রেজিস্ট্রেশন সফলভাবে জমা হয়েছে। আমাদের টিম শীঘ্রই আপনার আবেদন পর্যালোচনা করবে।
            </p>

            <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="background:#F7F8FC;border-radius:12px;margin-bottom:20px;">
              <tr><td style="padding:16px 18px;font-size:13.5px;color:#1F2432;">
                <div style="margin-bottom:8px;"><span style="color:#6B7280;">প্রতিষ্ঠানের নাম:</span> <strong>{{ $institution->name }}</strong></div>
                <div style="margin-bottom:8px;"><span style="color:#6B7280;">প্যাকেজ:</span> <strong>{{ ['basic'=>'বেসিক','standard'=>'স্ট্যান্ডার্ড','premium'=>'প্রিমিয়াম'][$institution->plan] ?? $institution->plan }}</strong></div>
                <div><span style="color:#6B7280;">প্রত্যাশিত ওয়েব ঠিকানা:</span> <strong>{{ $institution->slug }}.edution.xyz</strong></div>
              </tr></td>
            </table>

            <p style="margin:0 0 4px;font-size:14px;line-height:1.8;color:#6B7280;">
